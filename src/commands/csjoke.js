...
function synthesizeSpeech(text, output, language, voiceName) {
    ...
    var audioConfig = sdk.AudioConfig.fromAudioFileOutput(path.join(__dirname, `./temp/${output}`));
...
module.exports = {
    ...
    async execute(client, message) {
        ...
        let line2 = "Dear Dad; Do not dig in the field. That is where I hide that thing. You know I can not say what it is because they read our mail. Just do not dig out there. Your son";
        let line3 = "Dear son; The cops came out and dug up my fields. They said they were looking for something. Thanks, son. It looks like I will get the crops planted. Your loving and grateful father";
        ...
        dispatcher.on('finish', async() => {
                let dispatcher2 = await connection.play(path.join(__dirname, './temp/output2.wav'), {
                    volume: 0.8
                });

                dispatcher2.on('finish', async() => {
                        let dispatcher3 = await connection.play(path.join(__dirname, './temp/output3.wav'), {
                            volume: 0.8
                        });
                        dispatcher3.on('finish', async() => {
                                voiceChannel.leave();
